...
exports.isDebugMode = exports.isBrowser = void 0;
...
var _jsHtmlencode = _interopRequireDefault(require("../libs/js-htmlencode.js"));
var _debugger = require("../dbg/debugger.mjs");
function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }
class RazorError extends Error {
  constructor(message, captureFrame) {
    super(message);
    if (Error.captureStackTrace) Error.captureStackTrace(this, captureFrame || this.constructor);
  }
  static new(args) {
  ...
  static extend(exc, args) {
    ...
    if (!_debugger.isDebugMode) {
      exc.html = () => {
        const errorRefUrl = _debugger.isBrowser ? "https://www.npmjs.com/package/razjs#example-2-handling-and-displaying-errors" : "https://github.com/DevelAx/RazorExpress/blob/master/docs/Debugging.md#production--development-modes";
        ...
        if (_debugger.isBrowser) return `<div style="color: red;">${(0, _jsHtmlencode.default)(exc.message)}</div><hr/>${error}`;else return error;
      };
      return;
    }
    if (exc.data) {
    ...
    exc.data = Object.assign({
      line: args.line,
      pos: args.pos,
      len: args.len
    }, args.info);
    if (exc.__dbg && exc.__dbg.pos) exc.data = Object.assign({
      posRange: {
        start: exc.__dbg.pos.start,
        end: exc.__dbg.pos.end
      }
    }, exc.data);
    if (oldData) exc.data.inner = oldData;
    if (!exc.html) exc.html = RazorError.prototype.html;
  }
  html() {
    let codeHtml = '',
      mainInfo = {
...
function stackToHtml(exc, data, mainInfo) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (fireFox) {
      ...
      if (nextLineExists && data.jshtml && data.posRange) {
        // This is likely HTML parsing error (not code runtime error).
        ...
        if (codePos !== -1 && codePos === errorCodeFragment.lastIndexOf(codeLine)) {
          ...
          // Include '@' symbol.
          if (data.posRange.start > 0 && data.jshtml[data.posRange.start - 1] === '@') data.posRange.start -= 1;
        }
      }
    ...
    let encodedLine = (0, _jsHtmlencode.default)(trim);
    let style = '';
    if (trim && trim !== '^' && !trim.startsWith("at ")) {
      ...
      if (mainInfo.title) mainInfo.title += '\r\n';
...
function dataToHtml(data, mainInfo) {
  ...
  if (data.jshtml) {
    ...
    if (isLastData && !hasErrorCoordinates && mainInfo.errorLine) {
      ...
      if (occur.num === 1) {
        let extend = 0;
        if (occur.pos > 0 && data.jshtml[occur.pos - 1] === '@') extend = 1; // Include the '@' symbol for beauty.
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (pos != null && typeof pos !== 'undefined') {
        if (pos < line.length) {
          let start = (0, _jsHtmlencode.default)(line.substring(0, pos));
          let one = (0, _jsHtmlencode.default)(line.substring(pos, pos + len));
          let end = (0, _jsHtmlencode.default)(line.substring(pos + len));
        ...
        let trim = line.trim();
        if (trim.length > 6 && trim.startsWith("<!--") && trim.endsWith("-->")) comment = "class='comment'";
        //htmlLine = `<span class="comment">${htmlEncode(trim)}</span>`;
      }
      html += `<li ${comment || highlight}><span>`;
      html += htmlLine ? htmlLine : (0, _jsHtmlencode.default)(line);
      html += "</span></li>";
...
exports.ParserErrorFactory = void 0;
var _RazorError = _interopRequireDefault(require("./RazorError.mjs"));
function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }
class ParserErrorFactory {
  ...
  unexpectedCharacter(ch, lineNum, posNum, line) {
    ...
    return _RazorError.default.new({
      ...
      line: lineNum,
      pos: posNum,
      capture: this.unexpectedCharacter
    });
  }
  unexpectedAtCharacter(lineNum, posNum) {
    ...
    return _RazorError.default.new({
      ...
      line: lineNum,
      pos: posNum,
      capture: this.unexpectedAtCharacter
    });
  }
  notValidStartOfCodeBlock(ch, lineNum, posNum) {
    ...
    return _RazorError.default.new({
      ...
      line: lineNum,
      pos: posNum,
      capture: this.notValidStartOfCodeBlock
    });
  }
  unexpectedEndOfFile(text) {
    ...
    return _RazorError.default.new({
      message,
      info: this.info,
      capture: this.unexpectedEndOfFile
    });
  }
  characterExpected(ch, line, pos) {
    ...
    return _RazorError.default.new({
      ...
      line,
      pos,
      capture: this.characterExpected
    });
  }
  characterExpectedAfter(ch, line, pos, after) {
    ...
    return _RazorError.default.new({
      ...
      line,
      pos,
      capture: this.characterExpectedAfter
    });
  }
  expressionMissingEnd(expr, ch, line, pos) {
    ...
    return _RazorError.default.new({
      ...
      line,
      pos,
      capture: this.expressionMissingEnd
    });
  }
  jsCodeBlockMissingClosingChar(line, codeFirstLine) {
    ...
    return _RazorError.default.new({
      ...
      info: this.info,
      line,
      capture: this.jsCodeBlockMissingClosingChar
    });
  }
  ...
  missingMatchingStartTag(tag, line, pos) {
    ...
    return _RazorError.default.new({
      ...
      pos,
      len: tag.length,
      capture: this.missingMatchingStartTag
    });
  }
  missingMatchingEndTag(tag, line, pos) {
    ...
    return _RazorError.default.new({
      ...
      pos,
      len: tag.length,
      capture: this.missingMatchingEndTag
    });
  }
  invalidExpressionChar(ch, line, pos, afterText) {
    ...
    return _RazorError.default.new({
      ...
      line,
      pos,
      capture: this.invalidExpressionChar
    });
  }
  ...
  whiteSpaceExpectedAfter(keyword, line, pos) {
    ...
    return _RazorError.default.new({
      ...
      line,
      pos,
      capture: this.whiteSpaceExpectedAfter
    }); // cannot be tested.
  }
  ...
  sectionNameExpectedAfter(keyword, line, pos) {
    ...
    return _RazorError.default.new({
      ...
      line,
      pos,
      capture: this.sectionNameExpectedAfter
    });
  }
  sectionNameCannotStartWith(ch, line, pos) {
    ...
    return _RazorError.default.new({
      ...
      line,
      pos,
      capture: this.sectionNameCannotStartWith
    });
  }
  sectionNameCannotInclude(ch, line, pos) {
    ...
    return _RazorError.default.new({
      ...
      line,
      pos,
      capture: this.sectionNameCannotInclude
    });
  }
  unexpectedLiteralFollowingTheSection(ch, line, pos) {
    ...
    return _RazorError.default.new({
      ...
      line,
      pos,
      capture: this.unexpectedLiteralFollowingTheSection
    });
  }
  sectionIsAlreadyDefined(sectionName, line, pos, viewFilePath) {
    ...
    return _RazorError.default.new({
      ...
      capture: this.sectionIsAlreadyDefined
    });
  }
  ...
  sectionsCannotBeNested(line, pos) {
    ...
    return _RazorError.default.new({
      ...
      line,
      pos,
      capture: this.sectionsCannotBeNested
    });
  }
  sectionIsNotFound(sectionName, filePath) {
    ...
    return _RazorError.default.new({
      message,
      info: this.info,
      capture: this.sectionIsNotFound
    });
  }
  sectionIsNotCompiled(sectionName, filePath) {
    ...
    return _RazorError.default.new({
      message,
      info: this.info,
      capture: this.sectionIsNotCompiled
    });
  }
  sectionsAlreadyRendered(sectionName, renderedBy, attemptedBy) {
    ...
    return _RazorError.default.new({
      message,
      info: this.info,
      capture: this.sectionsAlreadyRendered
    });
  }
  sectionNeverRendered(sectionName, viewPath) {
    ...
    return _RazorError.default.new({
      message,
      info: this.info,
      capture: this.sectionNeverRendered
    });
  }
  partialViewNotFound(partialView, searchedLocations) {
    ...
    return _RazorError.default.new({
      message,
      info: this.info,
      capture: this.partialViewNotFound
    });
  }
  errorReadingFile(error) {
    let message = `Reading file '${this.info.filename}' caused an error: ${error}`;
    let parserError = _RazorError.default.new({
      message,
      info: this.info,
      capture: this.errorReadingFile
    });
  ...
  partialLayoutNameExpected() {
    ...
    return _RazorError.default.new({
      message,
      info: this.info,
      capture: this.partialLayoutNameExpected
    });
  }
   ...
   */
  extendError(exc) {
    _RazorError.default.extend(exc, {
      info: this.info
    });
  }
}
exports.ParserErrorFactory = ParserErrorFactory;
...
function setInnerError(parserError, error) {
  if (error.message) parserError.inner = error;
}

},{"./RazorError.mjs":4}],6:[function(require,module,exports){
/**
...
var utils = _interopRequireWildcard(require("./utils.mjs"));
var dbg = _interopRequireWildcard(require("./dbg/debugger.mjs"));
var _HtmlString = require("./HtmlString.mjs");
var _jsHtmlencode = _interopRequireDefault(require("./libs/js-htmlencode.js"));
var _errorsEn = require("./errors/errors.en.mjs");
...
(function (process, window, global, module, require, compilePage, compilePageSync, navigator, undefined) {
    ...
    // User section.
    if (debugMode) this.__dbg = {
      viewName: args.filePath,
      template: args.template,
      pos: []
    };
    this.$ = this.layout = null;
    // Private
    ...
    this.raw = function (val) {
      ...
      if (!isVisibleValue(val))
        // 'undefined' can be passed when `Html.raw()` is used by user in the view, in this case it will be wrapped into `Html.ecnode()` anyway an it will call `Html.raw` passing 'undefined' to it.
        return;
      if (sectionName) {
        let sec = sections[sectionName][args.filePath];
        if (!sec.compiled)
          // it could have been compiled already if it's defined in a partial view which is rendred more than once
          sec.html += val;
      } else {
        args.html += val;
      }
    ...
    this.getEncoded = function (val) {
      if (!isVisibleValue(val)) return '';
      if (typeof val === "number" || val instanceof Number || val instanceof _HtmlString.HtmlString) return val;
      if (String.is(val)) return htmlEncode(val);
      return htmlEncode(val.toString());
    };
    this.body = function () {
      return new _HtmlString.HtmlString(args.bodyHtml);
    };
    this.section = function (name, required) {
      if (!args.filePath) throw new Error("'args.filePath' is not set.");
      let secGroup = sections[name];
      if (secGroup) {
        if (secGroup.renderedBy) throw args.er.sectionsAlreadyRendered(name, secGroup.renderedBy, args.filePath); // TESTME:

        let html = '';
        for (var key in secGroup) {
          if (secGroup.hasOwnProperty(key)) {
            let sec = secGroup[key];
            if (!sec.compiled) throw args.er.sectionIsNotCompiled(name, args.filePath); // [#3.2]

            html += sec.html;
          }
        }
        secGroup.renderedBy = args.filePath;
        return new _HtmlString.HtmlString(html);
      } else {
        if (required) throw args.er.sectionIsNotFound(name, args.filePath); // [#3.3]
      }
      return '';
    };
    this.getPartial = function (viewName, viewModel) {
      let compileOpt = {
        scope: args.scope,
        model: viewModel === undefined ? args.model : viewModel,
        // if is not set explicitly, set default (parent) model
        findPartial: args.findPartial,
        findPartialSync: args.findPartialSync,
        findPartialAsync: args.findPartialAsync,
        sections,
...
Html.__dbg.pos = null;`;
  ...
  class Parser {
    constructor(args) {
      ...
      this.args = args;
      this.er = new _errorsEn.ParserErrorFactory({
        filename: args.filePath,
        jshtml: args.template
      }, linesBaseNumber);
    }
    compile(done) {
      log.debug();
      ...
      compilePage(htmlObj, this.args.model, this.args.viewData, this.args.scope, debugMode, (err, html) => {
        if (err) return error(err, htmlObj.__dbg);
        try {
          this.checkSections();
        } catch (exc) {
    ...
    compileAsync() {
      return new Promise((accept, reject) => {
        ...
        compilePageAsync(htmlObj, this.args.model, this.args.viewData, this.args.scope, debugMode).then(html => {
          try {
            this.checkSections();
          } catch (exc) {
            return error(exc, htmlObj.__dbg);
          ...
          return accept(html);
        }).catch(err => error(err, htmlObj.__dbg));
        function error(err, dbg) {
          err.__dbg = dbg;
          var parserError = toParserError(err, errorFactory);
    ...
    getHtml(htmlArgs) {
      ...
      if (!js) {
        ...
        if (!isString) throw new Error(ErrorsFactory.templateShouldBeString);
        this.text = template;
    ...
    parseHtml(blocks, outerWaitTag) {
      ...
      for (var ch = this.pickChar(); ch; ch = this.pickChar()) {
        ...
        if (inComments) {
          if (ch === '-') {
            if (!tag || tag === '-') tag += ch;else tag = '';
          } else if (ch === '>') {
            if (tag === '--') inComments = false;
            tag = '';
          } else {
            tag = '';
          }
          ...
          if (tag) tag += ch;
          if (textQuotes.indexOf(ch) !== -1) {
            // it could be closing text qoutes
            if (quotes.length && quotes[quotes.length - 1] === ch) {
              quotes.pop(); // collasping quotes..
              inQuotes = false;
            }
          }
        } else if ((tag || inJs) && textQuotes.indexOf(ch) !== -1) {
          // it could be opening text qoutes within a tag's attributes or a JS-block
          quotes.push(ch);
          inQuotes = true;
          if (tag) tag += ch;
        } else if (ch === '-') {
          ...
          if (tag) {
            if (tag.length === 1 || tag.length === 2 && lastCh === '/' || tag.startsWith(docTypeName, 1)) {
              // tag should be at least '<a' or '<a/'
              tag = ''; // it was just text
            } else {
              tag += ch;
              let tagName = getTagName(tag);
              let tagKind = tag.startsWith("</") ? tagKinds.close : tag.endsWith("/>") || voidTags.includes(tagName.toUpperCase()) ? tagKinds.selfclose : tagKinds.open;
              if (tagKind === tagKinds.close) {
                let openTag = openTags.pop();
                if (openTag) {
                  // if we have an open tag we must close it before we can go back to the caller method
                  if (openTag.name.toUpperCase() !== tagName.toUpperCase()) throw this.er.missingMatchingStartTag(tag, this.lineNum, this.linePos() - tag.length + 1); // tested by "Invalid-HTML 1+, 2+, 7"
                  // else they are neitralizing each other..
                  if ("script".equal(tagName, true)) inJs = false;
                ...
                openTags.push({
                  ...
                  linePos: this.linePos() - tag.length + 1
                });
              } else {
        ...
        } else if (!openTags.length && ch === '}' && lastLiteral === '>' /* || !block.text*/) {
          // the close curly bracket can follow only a tag (not just a text)
          this.stepBack(blocks, 0);
          stop = true;
          break; // return back to the callee code-block..
        } else {
          // any other character
          if (tag) tag += ch; // tag's insides
        }
        if (isSpace) {
          if (ch === '\n') {
            lineLastLiteral = '';
            ...
            // it's a true- space or tab
            if (lineLastLiteral)
              // it's not the beginning of the current line
              block.append(ch);else
      ...
      if (!stop) this.flushPadding(blocks);
    ...
    parseHtmlInsideCode(blocks) {
      ...
      for (var ch = this.pickChar(); !stop && ch; ch = ch && this.pickChar()) {
        ...
        if (inComments) {
          ...
          if (this.pickNextChar() === '@') {
            // checking for '@@' that means just text '@'
            ch = this.fetchChar(); // skip the next '@'
          } else if (openTagName || tag) {
            // it must be an expression somewhere inside HTML
            this.fetchChar(); // skip current '@'
            this.parseCode(blocks);
            if (tag && (tag === '<' || tag === '</')) tag += '@' + blocks[blocks.length - 1].text + this.padding; // just to be considered as a tag later (for the case of '<@tag>')
          ...
          if (textQuotes.indexOf(ch) !== -1) {
            // it could be the closing text qoutes
          ...
          if (tag) throw this.er.unexpectedCharacter(ch, this.lineNum, this.line.length, this.line + ch); // tested by "Invalid-HTML 8"

          if (openTagName) {
            // it should be a close-tag or another nested html-block
            if (nextCh !== '/') {
          ...
          if (tag) {
            if (nextCh === '/') {
              // it can be only considered as html, only as a text-fragment, so parse it as the next level of html..
          ...
          if (tag) {
            ...
            if (openTagName) {
              if (tag.length > 2) {
                // it's a close-tag, at least `</a`
                let tagName = getTagName(tag);
                if (openTagName.toUpperCase() !== tagName.toUpperCase()) throw this.er.missingMatchingStartTag(tag, this.lineNum, this.linePos() - tag.length + 1); // tested by "Code 22"
              ...
              if (tag[tag.length - 2] === '/' || voidTags.includes(tagName.toUpperCase())) {
                ...
                // it's an open-tag, at least `<a>`
                if (tag[1] === '/')
                  // it's a close-tag, unexpected..
              ...
              } else throw this.er.tagNameExpected(this.lineNum, this.linePos()); // tested by "Code 28"
            }
            tag = ''; //  reset it & go on..
          }
        } else if (isSpace) {
          ...
          if (tag) {
            ...
            // even if it is '}' it will be considered as a plain text
            processInnerHtml.call(this);
            continue;
          } else {
      ...
      if (openTagName) throw this.er.missingMatchingEndTag(openTag, openTagLineNum, openTagPos); // tested by "Invalid-HTML 6", "Code 20", "Code 31"
    ...
    parseCode(blocks) {
      ...
      if (!ch) throw Error(this.er.endOfFileFoundAfterAtSign(this.lineNum, this.linePos())); // tests: "Code 39"
      ...
      for (var ch = this.pickChar(); ch; ch = this.pickChar()) {
        ...
        if (checkForBlockCode) {
          ...
          if (pos !== -1) {
            ...
            } else if (textQuotes.indexOf(ch) !== -1 && (ch !== '/' || lastCh !== '<')) {
              // it's some sort of text qoutes (exclude the case when ch='/' and it's a tag start '</')
            ...
            if (Char.isWhiteSpace(lastCh) && !Char.isWhiteSpace(ch)) {
              ...
              if (!['function', 'class', 'try'].some(e => e == op)) break; // [Code 63]: <span>@year is a leap year.</span>
            }
            if (!canExpressionEndWith(ch)) {
              if (Char.isWhiteSpace(ch) || ch === '{') {
                if (checkForSection.call(this)) return;else if (ch === '{') {
                  let op = block.text.trim();
                ...
                if (!nextCh || !canExpressionEndWith(nextCh)) break;
              } else {
                break;
              }
            }
        ...
        if (Char.isWhiteSpace(ch)) {
          if (!checkForBlockCode) this.padding += ch;
        } else {
          if (!checkForBlockCode) this.flushPadding(blocks);
          block.append(ch);
        }
        lastCh = ch;
        this.fetchChar();
      }
      if (wait) throw this.er.expressionMissingEnd('@' + block.text, wait, this.lineNum, this.linePos()); // Tests: "Code 42".

      if (!block.text) throw this.er.invalidExpressionChar(ch, this.lineNum, this.linePos(), this.line); // Seems to be impossible.
    ...
    parseJsBlock(blocks, block, operatorName) {
      ...
      let firstLine = this.line,
        firstLineNum = this.lineNum,
        trackFirstLine = true;
      let waitOperator = null,
        waitAcc = '',
        operatorExpectScope;
      for (var ch = this.pickChar(); !stop && ch; ch = this.pickChar()) {
        // pick or fetch ??
        if (trackFirstLine) {
          trackFirstLine = ch !== '\n';
          if (trackFirstLine) firstLine += ch;
        }
        skipCh = false;
        if (waitOperator && ch !== '<' && ch !== '}' && ch !== operatorExpectScope) {
          if (!Char.isWhiteSpace(ch)) {
            ...
            // there shouldn't be any spaces within the 'waitOperator'
            if (waitOperator === "while") throw this.er.wordExpected(waitOperator, this.lineNum, this.linePos() - waitAcc.length); // [Code 59]
          ...
          // if not (inText)
          if (!firstScope && ch !== '{') throw this.er.characterExpected('{', this.lineNum, this.linePos());
          if (operatorExpectScope && !Char.isWhiteSpace(ch) && ch !== operatorExpectScope) {
            if (!waitOperator) throw this.er.characterExpectedAfter(operatorExpectScope, this.lineNum, this.linePos(), operatorName); // [Code 58, Code 66.1, Code 67.1]
          }
          let pos = startScopes.indexOf(ch);
          // IF it's a start-scope literal
          if (pos !== -1) {
            ...
            if (endScopes.indexOf(ch) !== -1) {
              // IF it's an end-scope literal
              if (wait === ch) {
                wait = waits.pop(); // collasping scope..
                if (( /*!wait && */operatorName !== "if" || ch === firstScope)) {
                  if (ch === '}') {
                    // the last & closing scope..)
                    switch (operatorName) {
                      case "try":
                        waitOperator = "catch";
                        break;
            ...
            } else if (textQuotes.indexOf(ch) !== -1) {
              // it's some sort of text qoutes
              wait && waits.push(wait);
              wait = ch;
              inText = true; // put on waits-stack
      ...
      if (wait) throw this.er.jsCodeBlockMissingClosingChar(firstLineNum, firstLine); // tests: "Code 29"

      if (operatorExpectScope) throw this.er.characterExpectedAfter(operatorExpectScope, this.lineNum, this.linePos(), operatorName); // [Code 55]
    ...
    parseSection() {
      ...
      if (this.inSection) throw this.er.sectionsCannotBeNested(this.lineNum, sectionStartPos); // Tests: "Section 2".
      ...
      if (spaceCount < 1) throw this.er.whiteSpaceExpectedAfter("@" + _sectionKeyword, this.lineNum, this.linePos()); // unreachable due to previous function check
      ...
      for (ch = this.pickChar(); ch && !Char.isWhiteSpace(ch) && ch !== '{'; ch = this.pickChar()) sectionName += this.fetchChar();

      // validation of the section name ..
      if (sectionName.length === 0) throw this.er.sectionNameExpectedAfter("@" + _sectionKeyword, this.lineNum, this.linePos()); // Tests: "Section 3".

      if (!canSectionStartWith(sectionName[0])) throw this.er.sectionNameCannotStartWith(sectionName[0], this.lineNum, this.linePos() - sectionName.length); // Tests: "Section 5".

      for (var i = 1; i < sectionName.length; i++) {
        let c = sectionName[i];
        if (!canSectionContain(c)) throw this.er.sectionNameCannotInclude(c, this.lineNum, this.linePos() - sectionName.length + i); // Tests: "Section 6".
      }
      ...
      if (sections) {
        ...
        if (section) throw this.er.sectionIsAlreadyDefined(sectionName, this.lineNum, sectionNamePos, this.args.filePath); // Tests: "Section 8".
      } else {
        this.args.parsedSections[sectionName] = sections = {};
      }
      sections[this.args.filePath] = {
        name: sectionName,
      ...
      if (ch !== '{') throw this.er.unexpectedLiteralFollowingTheSection(ch, this.lineNum, this.linePos()); // Tests: "Section 7".
    ...
    pickChar() {
      if (this.pos < this.text.length) return this.text[this.pos];
      return '';
    }
    pickNextChar() {
      if (this.pos < this.text.length - 1) return this.text[this.pos + 1];
      return '';
    }
    fetchChar() {
      if (this.pos < this.text.length) {
        var ch = this.text[this.pos++];
        if (ch === '\n') this.line = '', this.lineNum++;else this.line += ch;
        return ch;
      }
      return '';
    }
    stepBack(blocks, count) {
      ...
      let block = blocks[blocks.length - 1];
      if (count > this.line.length || block.text.length < count) throw new Error(`this.stepBack(${count}) is out of range.`);
      var cut;
      if (count > 0) {
        this.pos -= count;
        cut = this.line.length - count;
        if (cut === 0) this.line = '';else this.line = this.line.substr(0, cut);
      }
      ...
      if (!block.text.length || block.type === blockType.code && String.isWhiteSpace(block.text)) {
        ...
        if (cut === 0) blocks.pop(); // remove the current block if it's empty
        else block.text = block.text.substr(0, cut);
      }
    }
    linePos() {
    ...
    removeEmptyBlock() {
      if (this.blocks.length && !this.blocks[this.blocks.length - 1].text) this.blocks.pop();
    }
    newBlock(type, blocks, name) {
      ...
      if (blocks.length) blocks[blocks.length - 1].posEnd = textPos;
  ...
  function getTagName(tag) {
    if (!tag || tag.length < 2) throw this.er.invalidHtmlTag(tag, this.pos, this.line);
    var tagName = '';
    for (var i = 1; i < tag.length; i++) {
      ...
      if (ch === '>') break;
      if (Char.isWhiteSpace(ch)) {
        if (tagName) break;else throw this.er.invalidHtmlTag(tag, this.pos - tag.len, this.line);
      }
      tagName += ch;
    }
    return tagName;
  ...
  function toParserError(err, errorFactory) {
    if (err.isRazorError) {
      ...
      if (pos > 0) err.stack = err.stack.substring(pos + 1);
    }
    if (!err.isRazorError || err.__dbg && err.__dbg.viewName !== (err.data && err.data.filename)) errorFactory.extendError(err);
    return err;
  }
  ...
  function prepareArgs(args) {
    if (args.length) {
      // it's called from `compileSync`
      if (String.is(args[0])) args = {
        template: args[0],
        model: args[1]
      }; // arguments are not passed as an object
      else args = args[0];
    }
    args.root = true;
    return args;
...
},{"./HtmlString.mjs":1,"./dbg/debugger.mjs":3,"./errors/errors.en.mjs":5,"./libs/js-htmlencode.js":6,"./utils.mjs":8,"vm":9}],8:[function(require,module,exports){
...
String.prototype.startsWithIC = String.prototype.startsWithIgnoreCase = function (str, pos) {
  pos = pos || 0;
  if (this.length - pos < str.length) return false;
  for (let i = 0; i < str.length; i++) if (this[i + pos].toLowerCase() !== str[i].toLowerCase()) return false;
  return true;
};
String.equal = function (s1, s2, ignoreCase, useLocale) {
  ...
  if (useLocale) {
    if (useLocale.length) return s1.toLocaleLowerCase(useLocale) === s2.toLocaleLowerCase(useLocale);else return s1.toLocaleLowerCase() === s2.toLocaleLowerCase();
  } else {
    if (s1.length !== s2.length) return false;
