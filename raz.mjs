  ...
  static extend(exc, args) {
    ...
    if (!_debugger.isDebugMode) {
      exc.html = () => {
        ...
        if (_debugger.isBrowser) return `<div style="color: red;">${(0, _jsHtmlencode.htmlEncode)(exc.message)}</div><hr/>${error}`;else return error;
      };
      return;
    }
    if (exc.data) {
...
function stackToHtml(exc, data, mainInfo) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    let encodedLine = (0, _jsHtmlencode.htmlEncode)(trim);
    let style = '';
  ...
  if (data.jshtml) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (pos != null && typeof pos !== 'undefined') {
        if (pos < line.length) {
          let start = (0, _jsHtmlencode.htmlEncode)(line.substring(0, pos));
          let one = (0, _jsHtmlencode.htmlEncode)(line.substring(pos, pos + len));
          let end = (0, _jsHtmlencode.htmlEncode)(line.substring(pos + len));
      ...
      html += htmlLine ? htmlLine : (0, _jsHtmlencode.htmlEncode)(line);
      html += "</span></li>";
...
(async function (process, window, global, module, require, compilePage, compilePageSync, navigator, undefined) {
    ...
    this.getEncoded = function (val) {
      ...
      if (String.is(val)) return (0, _jsHtmlencode.htmlEncode)(val);
      return (0, _jsHtmlencode.htmlEncode)(val.toString());
    };
    this.body = function () {
